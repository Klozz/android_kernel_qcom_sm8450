				  ...
				  const struct io_uring_sqe *sqe)
{
	...
	if (tr->flags & IORING_TIMEOUT_UPDATE_MASK) {
		...
		if (tr->flags & ~(IORING_TIMEOUT_UPDATE_MASK|IORING_TIMEOUT_ABS))
			return -EINVAL;
		if (get_timespec64(&tr->ts, (const struct __kernel_timespec __user *)u64_to_user_ptr((u64)sqe->addr2)))
			return -EFAULT;
	} else if (tr->flags) {
			   ...
			   bool is_timeout_link)
{
	...
	if (get_timespec64(&data->ts, (const struct __kernel_timespec __user *)u64_to_user_ptr((u64)sqe->addr)))
		return -EFAULT;
